...
#define PIPE_1_SPAWN 128
#define PIPE_2_SPAWN 128 + PIPE_DISTANCE
#define PIPE_3_SPAWN 128 + PIPE_DISTANCE + PIPE_DISTANCE
#define PIPE_4_SPAWN 128 + PIPE_DISTANCE + PIPE_DISTANCE + PIPE_DISTANCE
...
Game::Pipe pipe;
    ...
    void Game::initialise()
    {
        ...
        for (uint8_t i = 0; i < PIPE_ARRAY_SIZE; i++) {pipe.active = false;}
    }
    ...
    void Game::game()
    {
        ...
        for (uint8_t i = 0; i < PIPE_ARRAY_SIZE; i++)
        {
            if (pipes[i].active) {pipes[i].x -= pipeSpeed;}
            if (pipes[i].x == -14) {pipes[i].active = false;}
    ...
    void Game::drawGame()
    {
        ...
        ab.println(pipes[2].x);
        ab.println(pipes[3].x);
    }
